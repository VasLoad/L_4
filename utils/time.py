def convert_time_from_milliseconds(milliseconds: int) -> str:
    """
    Конвертирует время из мсек. в формат чч:мм:сс.

    Args:
        milliseconds: Время в мсек.
    ...
    if hours_str:
        result_time_str += hours_str.zfill(2)
    if minutes_str:
        result_time_str += minutes_str.zfill(2)
    else:
        if hours_str:
            minutes_str = "00:"
    ...
    if seconds_str:
        result_time_str += seconds_str.zfill(2)
    else:
        if minutes_str:
            seconds_str = "00"
